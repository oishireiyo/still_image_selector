...
if __name__ == '__main__':
    ...
    # クラスタリングの実行
    x_means = XMeans(n_init=10, random_state = 1).fit(np.c_[x, y])
    logger.info(x_means.labels_)
    logger.info(x_means.cluster_centers_)
    logger.info(x_means.cluster_log_likelihoods_)
    logger.info(x_means.cluster_sizes_)
    ...
    plt.xlim(0, 3)
    plt.ylim(0, 3)
    plt.title('Result of the modified x-means method [Ref]: Ishioka(2000)')
    plt.savefig('../outputs/aho.png')